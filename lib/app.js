...
function listen(sv, store) {
    ...
    var promptForRFID = function() {
        prompt.get(['rfid'], function (err, result) {
            ...
            if (result.rfid === currentRFID) {
                ...
                // Do we just need to play the same playlist or are we playing?
                return mopidy.playback.getState()
                    .then((state) => {
                        if (state !== 'playing') {
                            ...
                            console.warn('This RFID is already the current one playing. Pausing');
                        }
            ...
            mopidy.tracklist.clear()
                ...
                .then(() => {
                    promptForRFID();
                    return mopidy.tracklist.setRepeat(rfidObject.loop);
                })
                .then((data) => {
     ...
     * @param {*} uris 
     */
    function addTracksToPlaylist(uris, index) {
        return mopidy.tracklist.add(undefined, index, undefined, uris);
    }
